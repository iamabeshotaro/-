...
<style>
    ...
    .block-container { padding-top: 3rem !important; padding-bottom: 1rem !important; max-width: 100% !important; }
    div.stButton > button {
        border-radius: 12px !important; box-shadow: 0 2px 5px rgba(0,0,0,0.05) !important;
        border: 1px solid #e0e0e0 !important; font-weight: 600 !important; padding: 6px 4px !important;
        font-size: 14px !important; min-height: 44px !important; line-height: 1.3 !important;
        white-space: normal !important; word-break: break-word !important; transition: all 0.2s ease !important;
    }
    div.stButton > button:active { transform: scale(0.96); }
    @media (prefers-color-scheme: dark) {
        div.stButton > button { border: 1px solid #333 !important; box-shadow: 0 2px 5px rgba(255,255,255,0.02) !important; }
    }
    div[data-testid="stVerticalBlockBorderWrapper"] > div { padding: 1px !important; gap: 1px !important; background-color: transparent !important; border: none !important; }
    .confirmed-cell {
        background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); color: white; border-radius: 8px;
        box-shadow: 0 2px 5px rgba(0, 242, 254, 0.2); padding: 4px; font-size: 10px; text-align: center;
        font-weight: bold; height: 100%; min-height: 45px; display: flex; align-items: center; justify-content: center;
        word-break: break-word; line-height: 1.2; overflow: hidden;
    }
    .empty-cell { background-color: rgba(0,0,0,0.03); border-radius: 8px; border: 1px dashed rgba(0,0,0,0.1); min-height: 45px; }
    @media (prefers-color-scheme: dark) {
        .confirmed-cell { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); box-shadow: 0 2px 5px rgba(118, 75, 162, 0.3); }
        .empty-cell { background-color: rgba(255,255,255,0.03); border: 1px dashed rgba(255,255,255,0.1); }
    }
    .tt-header { text-align: center; font-size: 12px; font-weight: 700; margin-bottom: 2px; color: #555; }
    ...
    @media screen and (max-width: 768px) {
        ...
        div[data-testid="stHorizontalBlock"]:has(> div:nth-child(6)) { flex-direction: row !important; flex-wrap: nowrap !important; gap: 2px !important; width: 100% !important; }
        div[data-testid="stHorizontalBlock"]:has(> div:nth-child(6)) > div[data-testid="column"] { width: auto !important; flex: 1 1 0% !important; min-width: 0 !important; padding: 0 !important; }
        div[data-testid="stHorizontalBlock"]:has(> div:nth-child(6)) > div[data-testid="column"]:first-child { flex: 0.35 1 0% !important; }
        div[data-testid="stHorizontalBlock"]:has(> div:nth-child(6)) button { font-size: 8px !important; padding: 0 !important; min-height: 40px !important; border-radius: 6px !important; letter-spacing: -0.5px !important; }
        .confirmed-cell { font-size: 8px !important; min-height: 40px !important; padding: 2px !important; border-radius: 6px !important; letter-spacing: -0.5px !important; }
        .empty-cell { min-height: 40px !important; border-radius: 6px !important; }
...
if not st.session_state.logged_in:
    ...
    if auth_mode == "新規登録":
        if st.button("登録してはじめる", type="primary", use_container_width=True):
            if not user_input or not pass_input: st.error("入力してください")
            else:
                users = load_users()
                if user_input in users: st.error("既に使用されています")
                else:
                    users[user_input] = { "password": hash_pass(pass_input), "registered": {"春学期": {}, "秋学期": {}}, "bookmarks": [], "likes": [] }
                    save_users(users)
        ...
        if st.button("ログイン", type="primary", use_container_width=True):
            ...
            if user_input in users and users[user_input]["password"] == hash_pass(pass_input):
                st.session_state.logged_in = True; st.session_state.current_user = user_input
                st.session_state.registered = users[user_input]["registered"]; st.session_state.bookmarks = users[user_input]["bookmarks"]
                cookie_manager.set("current_user", user_input, max_age=2592000)
                time.sleep(0.5); st.rerun()
            else: st.error("⚠️ 間違っています")
    st.stop()
...
with st.sidebar:
    ...
    with st.expander("⚙️ アカウント設定"):
        ...
        if st.button("変更を保存", use_container_width=True):
            if new_name == st.session_state.current_user: st.warning("現在と同じ名前です")
            elif not new_name.strip(): st.error("入力してください")
            else:
                users = load_users()
                if new_name in users: st.error("その名前はすでに使われています")
                else:
                    old_name = st.session_state.current_user
                    users[new_name] = users.pop(old_name)
                    for u in users.values():
                        if old_name in u.get("likes", []):
                            u["likes"].remove(old_name); u["likes"].append(new_name)
                    save_users(users)
        ...
        if del_confirm:
            if st.button("本当に削除する", type="primary", use_container_width=True):
                users = load_users(); old_name = st.session_state.current_user
                if old_name in users: users.pop(old_name)
                for u in users.values():
                    if old_name in u.get("likes", []): u["likes"].remove(old_name)
                save_users(users); cookie_manager.delete("current_user")
...
# ==========================================
# 7. 共通関数 & シェア機能用コンポーネント
# ==========================================
def get_slot_pairs(course):
    d_list = str(course['曜日']).split(); p_list = str(course['時限']).split()
    return list(zip(d_list, p_list))

def toggle_register(semester, course):
    if isinstance(course, pd.Series): course = course.to_dict()
    cid = course['授業コード']
    if cid not in [b['授業コード'] for b in st.session_state.bookmarks]: st.session_state.bookmarks.append(course)
    reg = st.session_state.registered[semester]
    if cid in reg: del reg[cid]; return
    target_slots = get_slot_pairs(course); conflicts = []
    for r_cid, r_course in reg.items():
        if any(s in get_slot_pairs(r_course) for s in target_slots): conflicts.append(r_cid)
    for c in conflicts: del reg[c]; st.toast("⚠️ 重複授業を仮登録に戻しました")
    reg[cid] = course
...
# ★ 新規：ネイティブシェアを呼び出すHTMLコンポーネントの生成関数
def generate_share_component(registered_data, semester, current_user, credits, tweet_text):
    days = ["月", "火", "水", "木", "金"]
    js_text = tweet_text.replace('\n', '\\n')
    
    html_style = """
    <style>
        #export-area { width: 800px; padding: 40px; background: white; font-family: sans-serif; border-radius: 20px; color: #333; box-sizing: border-box; position: relative; }
        ...
        .app-name { font-size: 16px; color: #888; margin: 0; font-weight: 600; }
        .semester-name { font-size: 28px; font-weight: 800; margin: 5px 0 0 0; color: #1a1a1a; }
        .credits-badge { background: #e1f0ff; color: #007bff; padding: 8px 16px; border-radius: 20px; font-weight: 700; font-size: 14px; display: inline-block; margin-bottom: 8px; }
        .user-name { font-size: 16px; color: #555; margin: 0; font-weight: 500; text-align: right; }
        .grid { display: grid; grid-template-columns: 0.6fr repeat(5, 1fr); gap: 10px; }
        .day, .period, .cell { border-radius: 10px; display: flex; align-items: center; justify-content: center; text-align: center; font-weight: 600; line-height: 1.3; }
        .day { height: 40px; background: #f8f9fa; color: #777; font-size: 14px; }
        .period { height: 80px; color: #bbb; font-size: 24px; font-weight: 700; }
        .cell { height: 80px; font-size: 11px; padding: 6px; }
        .cell.empty { background: #fafafa; border: 2px dashed #eee; }
        .cell.filled { background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); color: white; font-size: 12px; font-weight: 700; }
        .watermark { position: absolute; bottom: 10px; right: 15px; font-size: 10px; color: #ddd; font-weight: bold; }
    </style>
    """
    
    grid_html = '<div class="grid"><div class="day"></div>'
    for d in days: grid_html += f'<div class="day">{d}</div>'
    for p in range(1, 7):
        grid_html += f'<div class="period">{p}</div>'
        for d in days:
            course = next((c for c in registered_data.get(semester, {}).values() if (d, str(p)) in get_slot_pairs(c)), None)
            if course:
                name = course['授業名'][:20] + '..' if len(course['授業名']) > 20 else course['授業名']
                grid_html += f'<div class="cell filled">{name}</div>'
            else: grid_html += '<div class="cell empty"></div>'
    grid_html += '</div>'

    full_html = f"""
    <div style="width: 0; height: 0; overflow: hidden; position: absolute; left: -10000px; top: -10000px;">
        {html_style}
        <div id="export-area">
            <div class="header">
                <div><p class="app-name">中央大学商学部 時間割シミュレーター</p><p class="semester-name">{semester} の時間割</p></div>
                <div style="text-align: right;"><div class="credits-badge">合計 {credits:.1f} 単位</div><p class="user-name">👤 {current_user}</p></div>
            </div>
            {grid_html}
            <div class="watermark">GENERATED BY 時間割概論</div>
        </div>
    </div>
    
    <button id="share-btn" style="width: 100%; background-color: #000000; color: white; border: none; padding: 12px; font-size: 14px; font-weight: bold; border-radius: 12px; cursor: pointer; box-shadow: 0 2px 5px rgba(0,0,0,0.2);">
        𝕏 画像を作成して自動でシェアする
    </button>
    <div id="status" style="text-align:center; font-size:12px; color:#666; margin-top:8px; font-family:sans-serif;"></div>
        ...
        document.getElementById('share-btn').addEventListener('click', async () => {{
            ...
            try {{
                ...
                canvas.toBlob(async function(blob) {{
                    const file = new File([blob], 'jikanwari_{semester}.png', {{ type: 'image/png' }});
                    const shareData = {{ text: "{js_text}", files: [file] }};
                    ...
                    // シェア未対応の環境（PC等）の場合は自動ダウンロードに切り替え
                    if (!shared) {{
                        status.innerText = '画像をダウンロードしました！Xで添付してね👇';
                        const link = document.createElement('a');
                        link.download = 'jikanwari_{semester}.png';
                        link.href = canvas.toDataURL('image/png');
                        link.click();
                    }}
                }});
            }} catch(e) {{ status.innerText = 'エラーが発生しました'; }}
        }});
    </script>
    """
    return full_html
...
if nav1.button("🗓️", type="primary" if st.session_state.current_page == "tt" else "secondary", use_container_width=True, help="マイ時間割"):
    st.session_state.current_page = "tt"; st.rerun()
...
if st.session_state.current_page == "tt":
    ...
    with col_h2: st.write(f"✅ **{current_credits:.1f} 単位**")

    if view_mode == "👀":
        with st.expander("📤 この時間割をシェア・保存する", expanded=True):
            app_url = "https://あなたのアプリのURL.streamlit.app" 
            tweet_text = f"{semester}の時間割を組みました！（計 {current_credits:.1f} 単位）\n中大生向け時間割アプリ「時間割概論」\n#春から中大 #中央大学商学部\n{app_url}"
            
            # HTMLコンポーネントの表示（シェアボタン）
            html_content = generate_share_component(st.session_state.registered, semester, st.session_state.current_user, current_credits, tweet_text)
            components.html(html_content, height=100)
            
            # PCやブラウザ非対応時用のフォールバックボタン
            st.caption("※上のボタンで自動メニューが開かない場合は、ダウンロードされた画像を保存し、下のボタンから開いたXに添付してください👇")
            tweet_url = f"https://twitter.com/intent/tweet?text={urllib.parse.quote(tweet_text)}"
            st.link_button("𝕏 ブラウザからX(Twitter)を開く", tweet_url, use_container_width=True)
        ...
        if st.session_state.active_slot:
            d = st.session_state.active_slot['day']; p = st.session_state.active_slot['period']
            col_title, col_close = st.columns([4, 1])
            col_title.subheader(f"⚙️ {d}曜{p}限")
            if col_close.button("✖ 戻る", type="primary"): st.session_state.active_slot = None; st.rerun()
            ...
            for _, row in slot_courses.head(30).iterrows():
                with st.container(border=True):
                    ...
                    if b1.button("解除" if is_reg else "✅ 本登録", key=f"reg_{row['授業コード']}"):
                        toggle_register(semester, row.to_dict()); st.session_state.active_slot = None; save_and_rerun()
            ...
            with st.expander("＋ オリジナルの授業を作成する"):
                with st.form(f"custom_course_{d}_{p}"):
                    ...
                    if st.form_submit_button("✅ このコマに登録"):
                        if not c_name.strip(): st.error("授業名を入力してください")
                        else:
                            custom_id = f"MY_{int(time.time())}"
                            custom_course = { "授業コード": custom_id, "授業名": c_name, "担当教員": c_teacher if c_teacher else "不明", "単位数": f"{c_credits}単位", "曜日": d, "時限": str(p), "学期": semester, "詳細URL": "不明", "みんキャン検索LINK": "不明", "優先度": 99 }
                            toggle_register(semester, custom_course); st.session_state.active_slot = None; save_and_rerun()
        else:
            days = ["月", "火", "水", "木", "金"]
            cols = st.columns([0.6, 1, 1, 1, 1, 1])
            for i, d in enumerate([""] + days): cols[i].markdown(f"<div class='tt-header'>{d}</div>", unsafe_allow_html=True)
                        ...
                        with st.container(border=True):
                            ...
                            if bks_in_cell:
                                for b in bks_in_cell:
                                    cid = b['授業コード']; is_reg = cid in st.session_state.registered[semester]
                                    ...
                                    if st.button(btn_label, key=f"tt_{d}_{p}_{cid}", use_container_width=True, type="primary" if is_reg else "secondary"):
                                        toggle_register(semester, b); save_and_rerun()
                                if st.button("＋", key=f"add_{d}_{p}", use_container_width=True):
                                    st.session_state.active_slot = {"day": d, "period": str(p)}; st.rerun()
                            else:
    ...
    for _, row in res.head(50).iterrows():
        with st.container(border=True):
            ...
            if c1.button("解除" if is_reg else "✅ 本登録", key=f"src_reg_{row['授業コード']}"):
                toggle_register(active_sem, row.to_dict()); save_and_rerun()
    ...
    for b in st.session_state.bookmarks:
        with st.container(border=True):
            ...
            if c1.button("解除" if is_reg else "✅ 本登録", key=f"bk_reg_{b['授業コード']}"):
                toggle_register(active_sem, b); save_and_rerun()
    ...
    users = load_users(); my_id = st.session_state.current_user; public_users = [u for u in users.keys() if u != my_id]
    if not public_users: st.info("まだ他のユーザーがいません。")
    else:
        ...
        if selected_user != "選択してください...":
            target_data = users[selected_user]; likes_list = target_data.get('likes', [])
            c_head1, c_head2 = st.columns([3, 2])
            with c_head1: p_sem = st.selectbox("表示する学期", ["春学期", "秋学期"])
            with c_head2:
                has_liked = my_id in likes_list; like_btn_text = f"❤️ {len(likes_list)}" if has_liked else f"🤍 いいね ({len(likes_list)})"
